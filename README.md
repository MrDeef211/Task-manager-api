...
**Тестирование кода, UML-диаграмма сущностей и Диаграмма таблиц БД** в процессе, было решено перенести их реализацию на потом, поскольку по многим предеметам по учёбе поджимают сроки сдачи лабораторных, вернусь к их реализации, когда сдам все лабы.

В планах сначала тесты, затем диаграмма таблиц, и в конце диаграмма классов, как самое сложное
___
...
Сервис запросов репозиторий минует и обращается к базе напримую, т.к. данные он не изменяет и на момент единственной проблемой было, что метод GetTaskById два раза реализуется, один раз в сервисе, а второй в репозитории, чтобы для валидации запрос сделать можно было. Но при попытке реализовать тесты вскрылось, что проверять сервис запросов в таком виде нереально, поэтому в планах переделать, переместив работу с данными в репозиторий.
___
...
Сервисы (пока их 2: сервис комманд и сервис запросов) реализованы через интерфейс, сожержат бизнес логику и можно прикрутить полноценную умную валидацию.
